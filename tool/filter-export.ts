...
async function run() {
  ...
  console.log(
    `Found ${outlines.length} subscriptions. Checking update dates...`,
  );
    ...
    await Promise.all(
      chunk.map(async (outline) => {
        ...
        if (!latestDate) {
          ...
          removedOutlines.push(
            `${title} [Last updated: ${latestDate.getFullYear()}] (${xmlUrl})`,
          );
        } else {
          activeOutlines.push(outline.toString());
        }
...
<outline title="Subscriptions">
  ...
  if (removedOutlines.length > 0) {
    ...
    for (const outline of removedOutlines.slice(0, 10)) {
      console.log(`  - ${outline}`);
    }
  }
}
